...
tibble(QTL2D_TGW, QTL2D_GL, QTL5A_TGW, QTL5A_GW, QTL6A_TGW, QTL6A_GL) %>% 
...
LOD_BLUP %>% 
  ...
  ggplot(aes(x = pos, y = LOD, color = Trait))+
    ...
    scale_color_manual(values = c("#ee8080", "darkolivegreen4", "#6CC2BD", "#d03161"), name = "Trait (BLUP)", 
...
LOD_BLUP %>% 
  ...
  ggplot(aes(x = pos, y = LOD, color = Trait))+
    ...
    theme_classic() +
    theme(text = element_text(size = 12, face = "bold"), legend.position = "bottom") + 
    scale_color_manual(values = c("gray48", "darkolivegreen4", "firebrick2", "#3564D4"), name = "Trait (BLUP)", 
                       labels = c("GL", "GW", "HD", "TGW")) +
    xlab("Chromosome 5A (Mbp)") +
    annotate("text", x=380, y=5, label= "QTgw.cnl-5A", color = "#3564D4", fontface = 'italic')  +
    annotate("text", x=600, y=11, label= "VRN-1", color = "firebrick2", fontface = 'italic') 
...
#svg(file="DHnoKASP.svg", plot=DH_noKASP, path = "/Users/ellataagen/Desktop/", width=10, height=7)
```
...
subset[complete.cases(subset), ] %>%  
  ...
  ggplot(aes(x = KASP_341510829, y = Trait_value, color = KASP_341510829)) +
    ...
    scale_color_manual(values = c("#3564D4", "mediumaquamarine"))+
    theme_classic() +
    theme(text = element_text(size = 12, face = "bold"), legend.position = "bottom") +
...
chr5Apheno %>% 
  ...
  ggplot(aes(x = chr5AS_status, y = Trait_value, color = KASP_341510829)) +
    ...
    scale_color_manual(values = c("#3564D4", "mediumaquamarine","orange"))+
    theme_classic() +
