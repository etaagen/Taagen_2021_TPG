...
HIFttest %>%
  ...
  scale_fill_manual(values = c("#3564D4", "mediumaquamarine"))+
...
emmip(TGW_int, chr5AS_consensus ~ KASP_341510829, CIs = TRUE) #5AS present = heavier grains, Opata = heavier grains - they're in tight linkage but not interacting
...
```{r distribution plots, message = FALSE, echo=FALSE}
...
b <- ggplot(data = shortarm, aes(x = TGW_BLUP, fill = chr5AS_consensus))+
  geom_histogram(position = "identity", bins = 20, alpha = 0.8) + 
  scale_fill_manual(values = c("mediumaquamarine","#3564D4"))+ 
  theme_classic() +
